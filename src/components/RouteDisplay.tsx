...
export default function RouteDisplay({ routeData, isCalculating }: Props) {
    ...
    const handleCopy = async () => {
        ...
        // Якщо є сучасний API і безпечний контекст (https або localhost)
        ...
        // Запасний варіант для http:// (наприклад, тест через IP в локальній мережі)
        try {
            const textArea = document.createElement("textarea");
            textArea.value = text;
            // Ховаємо текстове поле, щоб воно не ламало верстку
            ...
            if (successful) {
                ...
                alert("Не вдалося скопіювати текст. Ваш браузер блокує цю дію.");
            }
        } catch (err) {
